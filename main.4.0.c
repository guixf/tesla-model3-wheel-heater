...
void	Eeprom_system_init(void)
{
									...
									tmp_value[4] = 0xe5;//写入PWM值90%
									tmp_value[5] = 0xd9;//写入PWM值80%
...
//写入结束
					...
					if(ADC_HIGH_SET_PWM==0xff||ADC_HIGH_SET_PWM==0) {
								  ...
								  SendString(string);		
									sprintf(string,"LOW_PWM & HIGH_PWM is %d/255 & %d/255!\r\n",ADC_LOW_SET_PWM,ADC_HIGH_SET_PWM);
								  SendString(string);
...
*/													
								...
								default:
								  SendString("Command is wrong!\r\n");
									SendString("Please Tx 0xaa 0x01 0x11--0x15 0xXX 0xXX 0xbb ");
